...
#' @param raw_values TRUE if you want to plot the raw values instead of percentages.
#'
#' @return a plot which highlights the probable position of your sample on your trajectory.
...
plot_position <- function(sample_ppr,
  ...
  add_gene_segments <- function(genes, switching_genes) {
    for (gene_name in genes) {
      ...
      segments(idx, -3.9, idx, -0.5, lwd = 1, lty = 2)
      text(x = idx + 3, y = 1, labels = gene_name, srt = -20, cex = 0.86, pos = 2)
    }
  }
  ...
  plot_sample <- function(sample_flat, col, label, overlay, raw_values) {
    x_vals <- 1:100
    y_vals <- if (raw_values) sample_flat else (sample_flat / max(sample_flat) * 100)
    
    if (!overlay) {
      ylim <- if (raw_values) range(y_vals) else c(0, 110)
      plot(x = x_vals, y = y_vals, ylim = ylim, xlim = c(0, 100), 
           pch = 20, cex = 0.8, col = col, type = "l", 
           xlab = "Pseudo-Time Index", ylab = if (raw_values) "Raw ppr Score" else "ppr Score (%)", 
    ...
    segments(mid_max_idx, -3.9, mid_max_idx, y_vals[mid_max_idx], lwd = 1, lty = 2, col = col)
    text(x = mid_max_idx, y = y_vals[mid_max_idx], labels = label, col = col, pos = 3)
  }
