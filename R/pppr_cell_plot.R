...
#'
#' @param sample.pppr A PPPR_OBJECT of the sample you wish to plot
...
#' @export
#'
pppr_cell_plot <- function(sample.pppr, cell_idx = 1, col = "red", overlay = FALSE, genes_of_interest = NULL, switching_genes = NULL, true_position = FALSE, accuracy_data = NULL) {
  if (!overlay) {
    plot(x = 1:100,
         y = sample.pppr$cells_flat[cell_idx,],
         ylim = c(0,max(sample.pppr$cells_flat) + 10),
         xlim = c(0,100),
         pch = 20,
         cex = 0.8,
         col = col,
    ...
    segments(which.max(sample.pppr$cells_flat[cell_idx,]),
             -99999,
             which.max(sample.pppr$cells_flat[cell_idx,]),
             sample.pppr$cells_flat[cell_idx,][which.max(sample.pppr$cells_flat[cell_idx,])],
             lwd = 1,
             lty = 2,
             col = col)
    ...
    if(true_position == TRUE) {
      ...
      text(x = (which.max(sample.pppr$cells_flat[cell_idx,]) - accuracy_data$true_position_of_cells_timeIDX[cell_idx] )/2 +accuracy_data$true_position_of_cells_timeIDX[cell_idx],
           y = sample.pppr$cells_flat[cell_idx,][which.max(sample.pppr$cells_flat[cell_idx,])],
           labels = paste(abs(which.max(sample.pppr$cells_flat[cell_idx,]) - accuracy_data$true_position_of_cells_timeIDX[cell_idx]), "\nINACCURACY"),
           col = col,
    ...
    lines(x = 1:100,
          y = sample.pppr$cells_flat[cell_idx,],
          ylim = c(0,max(sample.pppr$cells_flat[cell_idx,]) + 10),
          xlim = c(0,100),
          pch = 20,
          cex = 0.8,
          col = col,
