...
reference_reduced       <- filter_gene_expression_for_switching_genes(sce@assays@data@listData$binary    , switchingGenes)
```

### Produce an estimate for the position on trajectory of each gene in each cell of a sample.
```{r eval=FALSE}
Mole21a.sce   <- create_racing_lines(Mole21a_reduced  , switchingGenes)
sce    <- create_racing_lines(reference_reduced   , switchingGenes)
```

### Accuracy
#### *As our samples are subsets of the reference dataset we can calculate the accuracy of GSS*
```{r eval=FALSE}
Mole21a_accuracy   <- score_gss_accuracy(Mole21a.sce, sce)
reference_accuracy    <- score_gss_accuracy(sce, sce)
```
...
```{r eval=FALSE}
gss_output_plot(Mole21a.sce, col = "red", overlay=FALSE, label = "Mole21a", genes_of_interest = c("ENSG00000248605","ENSG00000272168"))
```
...
```{r eval=FALSE}
racinglines_timeline(switchingGenes, reference_reduced, cell_idx = 1)
```

### Plot the distribution of inaccuracy of each sample:
```{r eval=FALSE}
hist(Mole21a_accuracy$inaccuracy  , breaks = 100)
